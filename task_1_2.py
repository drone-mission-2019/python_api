...
def main():
    ...
    vrep.simxSynchronousTrigger(clientID)
    vrep.simxSetFloatSignal(clientID, 'landup', 1, vrep.simx_opmode_oneshot)
    vrep.simxSetFloatSignal(clientID, 'landdown', 0, vrep.simx_opmode_oneshot)
    target_height = 0.5
    bounds = [[-5, 5], [-5, 5]]
    ...
    while True:
        ...
        while True:
            ...
            if flight_controller.getPosition('controller')[2] <= 1.0:
                vrep.simxSetFloatSignal(clientID, 'landdown', 1, vrep.simx_opmode_oneshot)
            if flight_controller.getPosition('controller')[2] <= 0.55:
                print("Stop")
                stop_flag = True
                break
        if stop_flag:
